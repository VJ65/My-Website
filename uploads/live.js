...
(function () {
  ...
  function fetchLive() {
    ...
    return fetch(API, { headers: { accept: 'application/json' }, cache: 'no-store', signal: ctrl.signal })
      ...
      .then(function (d) {
        ...
        var out = { blogs: d.blogs || [], projects: d.projects || [] };
  ...
  function save(data, token) {
    ...
    return fetch(API, {
      ...
      body: JSON.stringify({ blogs: (data && data.blogs) || [], projects: (data && data.projects) || [] }),
      signal: ctrl.signal
    }).then(function (res) {
      clearTimeout(t);
      if (!res.ok) throw new Error('save failed (' + res.status + ')');
      try { localStorage.setItem(CACHE_KEY, JSON.stringify({ blogs: data.blogs || [], projects: data.projects || [] })); } catch (e) {}
      return res.json();
